...
renderGames = function(from_view) {
  ...
  if (fantasy_col == -1) {
    for (var i = 5; i < th.cells.length; i++) {
      if (
        th.cells[i].innerText.includes("-") &&
        th.cells[i].innerText.length < 3
      ) {
        fantasy_col = i + 1;
        break;
      }
  ...
  if (
    fantasy_col == -1 &&
    !document.getElementById("subnav_AS").className.includes("Hidden")
  ) {
    //find games column
    th = table.rows[first_player_row];
    for (var i = 2; i < th.cells.length; i++) {
      if (
        th.cells[i].innerText.includes(":") &&
        !th.cells[i].innerText.includes("m")
      ) {
        fantasy_col = i - 1;
        break;
      }
  ...
  while (!cellText.includes("Starting Lineup Totals")) {
    ...
    if (
      ...
      !table.rows[row].cells[0].innerText.includes("IL") &&
      !table.rows[row].innerText.includes("Injured")
    ) {
      ...
      table.rows[row].cells[fantasy_col].style.backgroundColor = getColor(
        games
      );
...
countStats = function() {
  ...
  for (var i = 2; i < th.cells.length; i++) {
    if (
      th.cells[i].innerText.includes(":") &&
      !th.cells[i].innerText.includes("m") &&
      !th.cells[i].innerText.includes("Q")
    ) {
      games_col = i - 1;
      col = i + 1;
      break;
  ...
  //on opponent page
  if (
    document.getElementsByClassName("F-icon Fz-xs F-trade T-action-icon-trade")
      .length > 1 ||
    document.getElementsByClassName(
      "noactiondt-tradedeadlinepassed F-icon Fz-xs F-disabled T-action-icon-disabled-trade"
    ).length > 1
  ) {
    offset = true;
    //console.log("offset");
  ...
  while (header_cell.length > 0) {
    ...
    if (header_cell.includes("/")) {
      ...
      stats_all.cells[write].innerText =
        round_float(num, 1) + "/" + round_float(den, 1);
      stats_week.cells[write].innerText =
        round_float(weekly_num, 1) + "/" + round_float(weekly_den, 1);
      ...
      if (
        (header_cell.includes("FGM/A") &&
          table.rows[1].cells[col + 1].innerText.includes("FG%")) ||
        (header_cell.includes("FTM/A") &&
          table.rows[1].cells[col + 1].innerText.includes("FT%"))
      ) {
...
if (
  document.getElementById("team-card-info") != null &&
  currentUrl.indexOf(teamURLMatch) !== -1
) {
  render();
  //renderGames();
  //countStats();
